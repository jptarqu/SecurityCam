...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    //JPT
    private var seconds = 60 //This variable will hold a starting value of seconds. It could be any amount above 0.
...
//
//            print("in closue of timer")
//        })
        
        self.toggleMovieRecordingForLater()
//        sessionQueue.async {
...
//                print("going to toggleMovieRecordingForLater")
...
//        }
    ...
    private func toggleMovieRecordingForLater() {
        guard let movieFileOutput = self.movieFileOutput else {
            return
        }
        print("in toggleMovieRecordingForLater")
        let videoPreviewLayerOrientation = previewView.videoPreviewLayer.connection?.videoOrientation
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true, block: { timer in
            ...
            print("in closue of timer")
            self.sessionQueue.async {
                print("in sessionQueue.async")
                
                print(movieFileOutput.isRecording)
                if movieFileOutput.isRecording {
                    print("movieFileOutput stopRecording")
                    movieFileOutput.stopRecording()
                }
                else {
                    print("in else")
                    ...
                    let outputFilePath = (NSTemporaryDirectory() as NSString).appendingPathComponent((outputFileName as NSString).appendingPathExtension("mov")!)
                    print(outputFileName)
                    print("movieFileOutput startRecording")
                    movieFileOutput.startRecording(to: URL(fileURLWithPath: outputFilePath), recordingDelegate: self)
                }
            }
